...
export const FriendRequestCard = ({friendRequest, setFriendRequest}) => {
    const {userInfo} = useContext(LoginContext)
    const token = userInfo.access_token
   
    const acceptRequest = async() => {
        try {
            await axios.put(`${BASE_URL}/friends/${friendRequest._id}/accept`
                ,{
                headers : {
                    'Authorization' : `Bearer ${token}`
                }
            })
            setFriendRequest(currentFriendRequest => currentFriendRequest.filter(el => el.playerId !== friendRequest.playerId))
        ...
        <View className={`px-4 py-2 w-5/6 mx-auto mt-8 rounded-lg bg-white shadow-lg`}>
            <View className={`flex-row items-center`}>
                ...
                <View className={`flex-1 justify-center`}>
                    <Text className={`text-lg`}>{`${friendRequest.name} mengirim permintaan pertemanan `}
                    </Text>
